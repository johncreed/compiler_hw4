...
void initializeSymbolTable() {
    ...
    floatAttribute->attr.typeDescriptor =
        (TypeDescriptor *)malloc(sizeof(TypeDescriptor));
    ...
    voidAttribute->attr.typeDescriptor =
        (TypeDescriptor *)malloc(sizeof(TypeDescriptor));
    ...
    readAttribute->attr.functionSignature =
    ...
    freadAttribute->attr.functionSignature =
        (FunctionSignature *)malloc(sizeof(FunctionSignature));
...
SymbolTableEntry *enterSymbol(char *symbolName, SymbolAttribute *attribute) {
    ...
    while (hashChain) {
        if (strcmp(hashChain->name, symbolName) == 0) {
            if (hashChain->nestingLevel == symbolTable.currentLevel) {
                printf("void enterSymbol(...): ID \'%s\' is redeclared(at the "
                       "same level#%d).\n",
    ...
    newEntry->nextInSameLevel =
        symbolTable.scopeDisplay[symbolTable.currentLevel];
...
void removeSymbol(char *symbolName) {
    ...
    while (hashChain) {
        if (strcmp(hashChain->name, symbolName) == 0) {
            if (hashChain->nestingLevel != symbolTable.currentLevel) {
                printf("void removeSymbol(...) Error: try to removed ID \'%s\' "
                       "from the scope other than currentScope.\n",
                       symbolName);
                return;
            } else {
                removeFromHashTrain(hashIndex, hashChain);
                if (hashChain->sameNameInOuterLevel) {
                    enterIntoHashTrain(hashIndex,
                                       hashChain->sameNameInOuterLevel);
                }
                break;
            }
        } else {
    ...
    if (!hashChain) {
        printf("void removeSymbol(...) Error: try to removed ID \'%s\' not in "
               "the symbol table.\n",
               symbolName);
        return;
    }
...
void openScope() {
    ...
    if (symbolTable.currentLevel == symbolTable.scopeDisplayElementCount) {
        ...
        symbolTable.scopeDisplay =
            (SymbolTableEntry **)malloc(symbolTable.scopeDisplayElementCount *
                                        2 * sizeof(SymbolTableEntry *));
        memcpy(symbolTable.scopeDisplay, oldScopeDisplay,
               symbolTable.scopeDisplayElementCount *
                   sizeof(SymbolTableEntry *));
        int index = 0;
        for (index = symbolTable.scopeDisplayElementCount;
             index != symbolTable.scopeDisplayElementCount * 2; ++index) {
            symbolTable.scopeDisplay[index] = NULL;
        }
        symbolTable.scopeDisplayElementCount =
            2 * symbolTable.scopeDisplayElementCount;
...
void closeScope() {
    if (symbolTable.currentLevel < 0) {
        printf("void closeScope(): Error: current level < 0. No scope can be "
               "close.\n");
        return;
    }
    SymbolTableEntry *scopeChain =
