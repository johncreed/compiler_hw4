...
void printErrorMsg(AST_NODE *node, ErrorMsgKind errorMsgKind) {
    ...
    case RETURN_ARRAY:
        printf("Function \'%s\' cannot return array.\n",
               node->rightSibling->semantic_value.identifierSemanticValue
                   .identifierName);
        break;
    case VOID_VARIABLE:
...
void processTypeNode(AST_NODE *idNodeAsType) {
    ...
    if ((symbolTableEntry == NULL) ||
        (symbolTableEntry->attribute->attributeKind != TYPE_ATTRIBUTE)) {
        printErrorMsg(idNodeAsType, SYMBOL_IS_NOT_TYPE);
        idNodeAsType->dataType = ERROR_TYPE;
    } else {
        idNodeAsType->semantic_value.identifierSemanticValue.symbolTableEntry =
            symbolTableEntry;

        switch (symbolTableEntry->attribute->attr.typeDescriptor->kind) {
        case SCALAR_TYPE_DESCRIPTOR:
            idNodeAsType->dataType = symbolTableEntry->attribute->attr
                                         .typeDescriptor->properties.dataType;
            break;
        case ARRAY_TYPE_DESCRIPTOR:
            idNodeAsType->dataType =
                symbolTableEntry->attribute->attr.typeDescriptor->properties
                    .arrayProperties.elementType;
            break;
        }
        //*/
    }
...
void declareIdList(AST_NODE *declarationNode,
    ...
    while (traverseIDList) {
        if (declaredLocally(traverseIDList->semantic_value
                                .identifierSemanticValue.identifierName)) {
            ...
            case NORMAL_ID:
                ...
                setIdentificationOffset(traverseIDList, attribute,
                                        sizeToAllocated);
                break;
            case ARRAY_ID:
                if ((isVariableOrTypeAttribute == TYPE_ATTRIBUTE) &&
                    (typeDescriptorOfTypeNode->kind ==
                ...
                processDeclDimList(traverseIDList,
                                   attribute->attr.typeDescriptor,
                ...
                if (traverseIDList->dataType == ERROR_TYPE) {
                    ...
                    declarationNode->dataType = ERROR_TYPE;
                               ...
                               .typeDescriptor->kind ==
                           SCALAR_TYPE_DESCRIPTOR) {
                    attribute->attr.typeDescriptor->properties.arrayProperties
                        .elementType =
                        typeNode->semantic_value.identifierSemanticValue
                            .symbolTableEntry->attribute->attr.typeDescriptor
                            ->properties.dataType;
                    sizeToAllocated = type_size *
                                      attribute->attr.typeDescriptor->properties
                                          .arrayProperties.total_size;
                    setIdentificationOffset(traverseIDList, attribute,
                                            sizeToAllocated);
                } else if (typeNode->semantic_value.identifierSemanticValue
                               .symbolTableEntry->attribute->attr
                               .typeDescriptor->kind == ARRAY_TYPE_DESCRIPTOR) {
                    int typeArrayDimension =
                        typeNode->semantic_value.identifierSemanticValue
                            .symbolTableEntry->attribute->attr.typeDescriptor
                            ->properties.arrayProperties.dimension;
                    int idArrayDimension =
                        attribute->attr.typeDescriptor->properties
                            .arrayProperties.dimension;
                    if ((typeArrayDimension + idArrayDimension) >
                        MAX_ARRAY_DIMENSION) {
                        printErrorMsg(traverseIDList,
                                      EXCESSIVE_ARRAY_DIM_DECLARATION);
                        free(attribute->attr.typeDescriptor);
                        traverseIDList->dataType = ERROR_TYPE;
                        declarationNode->dataType = ERROR_TYPE;
                        ...
                        int indexType = 0;
                        int indexId = 0;
                        for (indexType = 0, indexId = idArrayDimension;
                             indexId < idArrayDimension + typeArrayDimension;
                             ++indexType, ++indexId) {
                            attribute->attr.typeDescriptor->properties
                                .arrayProperties.sizeInEachDimension[indexId] =
                                typeNode->semantic_value.identifierSemanticValue
                                    .symbolTableEntry->attribute->attr
                                    .typeDescriptor->properties.arrayProperties
                                    .sizeInEachDimension[indexType];
                            /* TODO set offset for array declare i.e. int[2]
                            a[2]
                            sizeToAllocated = type_size *
                            attribute->attr.typeDescriptor->
                ...
                break;
            case WITH_INIT_ID:
                if (typeNode->semantic_value.identifierSemanticValue
                        .symbolTableEntry->attribute->attr.typeDescriptor
                        ->kind == ARRAY_TYPE_DESCRIPTOR) {
                ...
                break;
            default:
                printf("Unhandle case in void declareIdList(AST_NODE* "
                       "typeNode)\n");
                traverseIDList->dataType = ERROR_TYPE;
                declarationNode->dataType = ERROR_TYPE;
                break;
            ...
            if (traverseIDList->dataType == ERROR_TYPE) {
                ...
                traverseIDList->semantic_value.identifierSemanticValue
                    .symbolTableEntry =
                    enterSymbol(traverseIDList->semantic_value
                                    .identifierSemanticValue.identifierName,
                                attribute);
            }
        }
...
void checkAssignmentStmt(AST_NODE *assignmentNode) {
    ...
    if (rightOp->dataType == INT_PTR_TYPE ||
        ...
        assignmentNode->dataType =
            getBiggerType(leftOp->dataType, rightOp->dataType);
    }
}
...
void checkParameterPassing(Parameter *formalParameter,
    ...
    if (actualParameter->dataType == INT_PTR_TYPE ||
        actualParameter->dataType == FLOAT_PTR_TYPE) {
        actualParameterIsPtr = 1;
    }

    if (formalParameter->type->kind == SCALAR_TYPE_DESCRIPTOR &&
        actualParameterIsPtr) {
        printErrorMsgSpecial(actualParameter, formalParameter->parameterName,
                             PASS_ARRAY_TO_SCALAR);
...
void getExprOrConstValue(AST_NODE *exprOrConstNode, int *iValue,
    ...
    if (exprOrConstNode->nodeType == CONST_VALUE_NODE) {
        if (exprOrConstNode->dataType == INT_TYPE) {
            if (fValue) {
                *fValue =
                    exprOrConstNode->semantic_value.const1->const_u.intval;
            } else {
                *iValue =
                    exprOrConstNode->semantic_value.const1->const_u.intval;
            }
        ...
        if (exprOrConstNode->dataType == INT_TYPE) {
            if (fValue) {
                *fValue = exprOrConstNode->semantic_value.exprSemanticValue
                              .constEvalValue.iValue;
            } else {
                *iValue = exprOrConstNode->semantic_value.exprSemanticValue
                              .constEvalValue.iValue;
            }
        } else {
            *fValue = exprOrConstNode->semantic_value.exprSemanticValue
...
void evaluateExprValue(AST_NODE *exprNode) {
    if (exprNode->semantic_value.exprSemanticValue.kind == BINARY_OPERATION) {
        ...
        if (leftOp->dataType == INT_TYPE && rightOp->dataType == INT_TYPE) {
            ...
            case BINARY_OP_ADD:
                exprNode->semantic_value.exprSemanticValue.constEvalValue
            ...
            case BINARY_OP_OR:
                ...
                break;
            default:
                printf("Unhandled case in void evaluateExprValue(AST_NODE* "
                       "exprNode)\n");
                break;
            }
...
void processExprNode(AST_NODE *exprNode) {
    if (exprNode->semantic_value.exprSemanticValue.kind == BINARY_OPERATION) {
        ...
        processExprRelatedNode(rightOp);
        // special case
        if (leftOp->dataType == INT_PTR_TYPE ||
            leftOp->dataType == FLOAT_PTR_TYPE) {
        ...
        if (leftOp->dataType == CONST_STRING_TYPE ||
            rightOp->dataType == CONST_STRING_TYPE) {
        ...
        if ((exprNode->dataType != ERROR_TYPE) &&
            (leftOp->nodeType == CONST_VALUE_NODE ||
             (leftOp->nodeType == EXPR_NODE &&
              leftOp->semantic_value.exprSemanticValue.isConstEval)) &&
            (rightOp->nodeType == CONST_VALUE_NODE ||
        ...
        // special case
        if (operand->dataType == INT_PTR_TYPE ||
            operand->dataType == FLOAT_PTR_TYPE) {
        ...
        if ((exprNode->dataType != ERROR_TYPE) &&
            (operand->nodeType == CONST_VALUE_NODE ||
...
void processVariableLValue(AST_NODE *idNode) {
    ...
    } else if (idNode->semantic_value.identifierSemanticValue.kind ==
               ARRAY_ID) {
        int dimension = 0;
...
void processVariableRValue(AST_NODE *idNode) {
    ...
    if (idNode->semantic_value.identifierSemanticValue.kind == NORMAL_ID) {
        if (typeDescriptor->kind == ARRAY_TYPE_DESCRIPTOR) {
            if (typeDescriptor->properties.arrayProperties.elementType ==
                INT_TYPE) {
        ...
        if (typeDescriptor->kind == SCALAR_TYPE_DESCRIPTOR) {
            ...
            if (idNode->dataType != ERROR_TYPE) {
                if (dimension ==
                    ...
                    idNode->dataType =
                        typeDescriptor->properties.arrayProperties.elementType;
                } else if (dimension > typeDescriptor->properties
                                           .arrayProperties.dimension) {
                    printErrorMsg(idNode, INCOMPATIBLE_ARRAY_DIMENSION);
                    idNode->dataType = ERROR_TYPE;
                } else if (typeDescriptor->properties.arrayProperties
...
void checkReturnStmt(AST_NODE *returnNode) {
    ...
    while (parentNode) {
        if (parentNode->nodeType == DECLARATION_NODE) {
            if (parentNode->semantic_value.declSemanticValue.kind ==
                FUNCTION_DECL) {
    ...
    if (returnNode->child->nodeType == NUL_NODE) {
        ...
        if (returnType != returnNode->child->dataType) {
            if (!((returnType == FLOAT_TYPE &&
                   returnNode->child->dataType == INT_TYPE) ||
                  (returnType == INT_TYPE &&
                   returnNode->child->dataType == FLOAT_TYPE))) {
                errorOccur = 1;
            }
        }
    }
...
void processDeclDimList(AST_NODE *idNode, TypeDescriptor *typeDescriptor,
    ...
    if (ignoreFirstDimSize && traverseDim->nodeType == NUL_NODE) {
        typeDescriptor->properties.arrayProperties
            .sizeInEachDimension[dimension] = 0;
        ++dimension;
    ...
    while (traverseDim) {
        ...
        if (traverseDim->dataType == ERROR_TYPE) {
            ...
            typeDescriptor->properties.arrayProperties
                ...
                traverseDim->semantic_value.exprSemanticValue.constEvalValue
                    .iValue;
            typeDescriptor->properties.arrayProperties.total_size *=
                typeDescriptor->properties.arrayProperties
                    .sizeInEachDimension[dimension];
        }
